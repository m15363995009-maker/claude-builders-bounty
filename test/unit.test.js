...
const { main, parseArgs, validateFixture } = require("../bin/claude-review");
...
assert.throws(
  () => parseArgs(["--pr", "https://github.com/example/project/pull/1", "--fixture", "fixture.json"]),
  /cannot be used together/,
);
assert.throws(
  () => parseArgs(["--fixture", "fixture.json", "--post-comment"]),
  /requires a live --pr/,
);
assert.throws(() => validateFixture({ diff: "" }), /files array/);
...
async function testOfflineFixtureMode() {
  let networkCalls = 0;
  let output = "";
  const result = await main({
    ...
    runClaudeReviewImpl: () => {
      throw new Error("heuristic fixture mode must not call Claude");
    },
  });
  assert.equal(networkCalls, 0);
  assert.deepEqual(result, { action: "reviewed", engine: "heuristic" });
  assert.match(output, /## Summary of changes/);
  assert.match(output, /Generated by claude-review \(heuristic engine\)/);
}

Promise.all([testCommentPosting(), testOfflineFixtureMode()])
